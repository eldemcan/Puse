 ...
 * (http://purl.eligrey.com/github/classList.js/blob/master/classList.js) */
 ...
 (function (view) {
	...
	function reset() {
		...
		obj.className = obj.className.replace(/ *warning/g,"").replace(/ *deadline/, "");
		updateText(obj)
      ...
      if (obj.className.search('advance') != -1) {
	...
	function countdown() {
		...
		if (count > 0) {
			count--;
			if (count == warning) {
				obj.className = obj.className + ' warning';
			}
		} else {
			obj.className = obj.className.replace(/ *warning/,"") + ' deadline';
			obj.onclick = reset
		}
	}
